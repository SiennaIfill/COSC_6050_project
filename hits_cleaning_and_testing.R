...
Nat_hits <- filter(hits, Name == "Natalie Ring")
#get kill pct
...
top_4 <- Nat_hits |> count(AttackPlay, skill_subtype, end_zone, sort = TRUE) |> slice(1:4)
View(top_4)
...
top1_all <- Nat_hits |> filter(AttackPlay==top_4[[1,1]], skill_subtype==top_4[[1,2]],end_zone==top_4[[1,3]])
...
top2_all <- Nat_hits |> filter(AttackPlay==top_4[[2,1]], skill_subtype==top_4[[2,2]],end_zone==top_4[[2,3]])
...
top3_all <- Nat_hits |> filter(AttackPlay==top_4[[3,1]], skill_subtype==top_4[[3,2]],end_zone==top_4[[3,3]])
...
top4_all <- Nat_hits |> filter(AttackPlay==top_4[[4,1]], skill_subtype==top_4[[4,2]],end_zone==top_4[[4,3]])
